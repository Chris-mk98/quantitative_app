...
class QuantitativeUI:
    ...
    def build_ui(self):
        ...
        ttk.Button(frame, text="Raw 파일 선택", command=self.select_file).grid(row=1, column=0)
        self.file_label = ttk.Label(frame, text="선택된 파일 없음")
        self.file_label.grid(row=1, column=1, sticky="w")


        ttk.Button(frame, text="기준 추가", command=self.add_row).grid(row=2, column=0, pady=5)

        self.table = ttk.Frame(frame)
        self.table.grid(row=3, column=0, columnspan=4, sticky="w")

        headers = ["순번", "분석계정", "X값", "X비교", "포함", "삭제"]
        for i, h in enumerate(headers):
            ttk.Label(self.table, text=h).grid(row=0, column=i)

        ttk.Button(frame, text="변환", command=self.on_convert).grid(row=4, column=0, pady=10)
    ...
    def add_row(self):
        ...
        xcompare = ttk.Combobox(
            self.table,
            values=["미만", "초과", "같음", "텍스트 일치", "All equals"],
            width=12
        )
    ...
    def delete_row(self, idx):
        for widget in self.table.grid_slaves(row=idx+1):
            widget.destroy()
    ...
    def resequence(self):
        for i, row in enumerate(self.rows):
            ...
            for widget in self.table.grid_slaves(row=i+2):
                widget.grid(row=i+1)
